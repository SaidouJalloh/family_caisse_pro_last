...
// AVEC STATISTIC
// src/App.tsx - Version avec statistiques intégrées
import React, { useState, useEffect, useCallback, useRef } from 'react'
import {
  Home,
  Users,
...
// Types
type NavigationSection = 'dashboard' | 'members' | 'cotisations' | 'payments' | 'activities' | 'statistics' | 'settings'
...
const App: React.FC = () => {
  ...
  // Navigation items (AVEC STATISTIQUES)
  const navigationItems = React.useMemo((): NavigationItem[] => [
    {
      id: 'dashboard',
      name: 'Tableau de Bord',
    ...
    },
    {
      ...
      color: 'text-pink-600',
      bgColor: 'bg-pink-100',
      description: 'Rapports et analyse',
      roles: ['admin'] // Seulement pour les admins
    },
    {
      id: 'settings',
      name: 'Paramètres',
  ...
  const handleNavigation = useCallback((section: string) => {
    if (['dashboard', 'members', 'cotisations', 'payments', 'activities', 'statistics', 'settings'].includes(section)) {
      setCurrentSection(section as NavigationSection)
      setIsMobileMenuOpen(false)
    }
  }, [])
  ...
  // Rendu des sections (AVEC STATISTIQUES)
  const renderCurrentSection = useCallback(() => {
    if (!profile) return null
    ...
    switch (currentSection) {
      ...
      case 'statistics':
        return hasAccess(['admin']) ?
          <StatisticsPage onBack={() => handleNavigation('dashboard')} /> :
          <AccessDenied />
      case 'settings':
        return <Settings onNavigate={handleNavigation} />
      default:
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode
      ...
      {/* Navigation Mobile Bottom (AVEC STATISTIQUES) */}
      <div className={`lg:hidden fixed bottom-0 left-0 right-0 ${isDarkMode ? 'bg-gray-800/90' : 'bg-white/90'
        } backdrop-blur-lg border-t ${isDarkMode ? 'border-gray-700' : 'border-white/20'
        } shadow-lg z-40`}>
        <div className="flex items-center justify-around py-2">
          {/* Afficher les 4 premiers items + statistiques pour admin */}
          {[
            ...filteredNavigationItems.slice(0, 3),
            filteredNavigationItems.find(item => item.id === 'statistics'),
            filteredNavigationItems.find(item => item.id === 'settings')
